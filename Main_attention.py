...
from utils.c_utils import save_checkpoint, train, validate, train_att, validate_att
import cfg.config as cfg
from models.att_resnet import resnet101_att
import csv
import shutil
import time
...
writer = SummaryWriter('/home/yyang2/data/yyang2/Data/EyeQ-master/results/contract/tensorboard/res101_att')
...
model = resnet101_att(num_classes=cfg.n_class)
# 已有训练结果时进行加载
...
train_dataset = DatasetGenerator(list_file=cfg.train_file,
                              ...
                              transform2=transformList2, n_class=5, set_name='train',mode = 'attention')
...
test_dataset = DatasetGenerator(list_file=cfg.test_file,
                             ...
                             transform2=transformList2, n_class=5, set_name='val',mode = 'attention')
...
for epoch in range(cfg.start_epoches,cfg.epoches):
    ...
    acc_t1, kappa_t, loss_t = train_att(train_loader, model, criterion, optimizer, epoch, cycle_scheduler)


    # evaluate on validation set
    acc_v, kappa_v, loss_v = validate_att(val_loader, model, criterion)
    if (epoch + 1) % 10 == 0:
        acc_t, kappa_t, loss_t = validate_att(test_loader, model, criterion)
    ...
    filename = path_save + 'att_checkpoint.pth.tar'
    file_best_acc_name = path_save + 'acc/' + str(epoch + 1) + '_att_best_acc.pth.tar'
    file_best_kappa_name = path_save + 'kappa/' + str(epoch + 1) + '_att_best_acc.pth.tar'
