...
class HyperparameterScheduler():
    ...
    def batch_step(self):
        ...
        for hyperparam_name, milestones in self.hyperparam_milestones.items():
            value = np.array(np.interp(self.batch_count, self.ts, milestones))
            for param_group in self.optimizer.param_groups:
                param_group[hyperparam_name] = torch.from_numpy(value).type(torch.cuda.FloatTensor)
